...
function ChangeProfile() {
  ...
  const onSubmit = async (data) => {
    ...
    await axios
      .post("/api/user/ChangeProfilewithpic", formData)
      .then((response) => {
        if (response.data) {
      ...
      .catch((error) => {
        if (error.response) {
         ...
         await axios
      .post("/api/user/ChangeProfile", Data)
      .then((response) => {
        if (response.data) {
          toast.success("profile changed successful");
        }
        console.log(response.data)
        localStorage.setItem("ChatApp", JSON.stringify(response.data));
        setAuthUser(response.data);
    ...
    <>
      <div className="flex h-screen items-center justify-center">
        ...
        >
          ...
          <h2 className="text-xl text-white text-center font-bold">Change Profile</h2>
          <br />

          <div className="avatar flex justify-center">
            <div className="w-24 rounded-full">
            <img src={ previewImage ? previewImage : authUser.user.avatarUrl } />
            </div>
          </div>
          ...
          <label className="input input-bordered flex items-center gap-2">
            ...
            <input
              type="text"
              className="grow"
              placeholder = {authUser.user.fullname}
